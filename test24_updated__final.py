...
class ImageAnalyzerApp:
    def __init__(self, root):
        ...
        # ── App title — centered at top-middle of header bar ────────────
        # Use place() so the label is absolutely centered regardless of the
        # left-aligned nav frame and right-aligned logout button.
        tk.Label(header,
                 text='ADDITIVE MANUFACTURING DEFECT DETECTION',
        ...
        for key, label in tabs:
            ...
            # Change 5: Do NOT pack the 'batch' nav button — the batch content
            # is the default view and needs no dedicated nav button in the header.
            if key != 'batch':
    ...
    def _batch3d_start(self):
        ...
        # Change 4: Renamed output folders defects→indicator, non_defects→non_indicator
        defects_path     = os.path.join(output_base, 'indicator')
        non_defects_path = os.path.join(output_base, 'non_indicator')
        ...
        def _run():
            ...
            workers = max(1, min(_mp.cpu_count(), 4))
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for done_idx, fut in enumerate(_as_completed(futures), start=1):
                    ...
                    try:
                        ...
                        if num_seg >= 1:
                            defect_count += 1
                            # Change 2: emoji removed from msg
                        ...
                        # Change 2: emoji removed from error msg
    ...
    def _batch3d_set_report_link(self, report_path):
        ...
        if hasattr(self, '_batch3d_report_link_btn'):
            # Change 2: emoji removed from report link label
            self._batch3d_report_link_btn.config(
    ...
    def _batch3d_generate_report_auto(self, output_base, defects_path, non_defects_path, report_path,
        ...
        # Change 4: category keys renamed to 'indicator' / 'non_indicator'
        all_imgs = ([(f, 'indicator') for f in defect_imgs] +
                    [(f, 'non_indicator') for f in non_defect_imgs])
        ...
        # Change 4: category label map updated to new folder names
        CAT_LABELS = {'indicator': 'Indicator', 'non_indicator': 'Non Indicator'}
        ...
        for r_idx, (fname, cat) in enumerate(all_imgs, start=2):
            # Change 4: fill condition updated to new category key names
            fill = DEF_FILL if cat == 'indicator' else NOD_FILL
        ...
        if skip_generation:
            ...
            try:
                ...
                self.root.after(0, lambda e=_seg_copy_exc: self.tab3_log(
                    # Change 2: emoji removed
                    f"WARN: Could not stage seg.nrrd into 3D Models folder: {e}"))
                seg_out = seg_src  # fall back to loading it directly in place

            png_files = []
            category_summary = ''
        else:
            # Collect PNGs from indicator / non_indicator subfolders
            # Change 4: added 'indicator'/'non_indicator' to recognised sub-folder names
            png_entries = []
            for sub in sorted(os.listdir(folder)):
                sub_path = os.path.join(folder, sub)
                if os.path.isdir(sub_path) and sub.lower() in (
                        'defects', 'non_defects', 'defect', 'non_defect',
                        'indicator', 'non_indicator'):
            ...
            if not png_entries:
                self.root.after(0, lambda: self.tab3_log(
                    # Change 2: emoji removed
                    "WARN: No PNG files found for 3D Visualization."))
                self.root.after(0, lambda: self._batch3d_set_progress(100))
...
"""
        ...
        try:
            ...
            if os.path.isfile(stl_file):
                ...
                # Change 2: emoji removed
                self.root.after(0, lambda: self.tab3_log("WARN: STL file not found yet - skipping copy to 3D Models folder."))
        except Exception as _copy_exc:
            # Change 2: emoji removed
            self.root.after(0, lambda e=_copy_exc: self.tab3_log(f"WARN: Could not copy STL into 3D Models folder: {e}"))
        ...
        def _make_script(headless):
            ...
            import os, glob, subprocess, random, pathlib, vtk, slicer, numpy as np, qt
            ...
            def create_segmentation_from_png():
                ...
                print("STEP::1/14::Loading PNG Stack...")
                print("\\n=== Loading PNG stack ===")
                ...
                # Change 6: Parallelise PNG loading using ThreadPoolExecutor.
                # The sequential for-loop is replaced with a thread pool so that
                # image I/O for all slices happens concurrently, significantly
                # reducing load time for large stacks. The algorithm (stack shape,
                # resize logic, numpy conversion) is identical to the original.
                import concurrent.futures as _cf_slicer, os as _os_slicer
                _n_workers = max(1, min(_os_slicer.cpu_count() or 4,
                                        len(png_files), 8))
                # Determine target size from the first image (single read, cheap)
                _first_img = Image.open(png_files[0]).convert("RGB")
                target_size = _first_img.size

                def _load_slice(f):
                    """Load, optionally resize, and convert one PNG to ndarray."""
                    _img = Image.open(f).convert("RGB")
                    if _img.size != target_size:
                        _img = _img.resize(target_size)
                    return np.array(_img)

                print(f"Loading {{len(png_files)}} PNG(s) with {{_n_workers}} thread(s)...")
                with _cf_slicer.ThreadPoolExecutor(max_workers=_n_workers) as _pool:
                    slices = list(_pool.map(_load_slice, png_files))
                # End Change 6
                ...
                print("STEP::2/14::Extracting Red Mask...")
                ...
                print("STEP::3/14::Creating Scalar Volume...")
                ...
                print("STEP::4/14::Creating Segmentation...")
                ...
                print("STEP::5/14::Applying Threshold...")
                ...
                print("STEP::6/14::Splitting Islands...")
                isl.setParameter("Operation","SPLIT_ISLANDS_TO_SEGMENTS"); isl.self().onApply(); slicer.app.processEvents()
                ew.setActiveEffectByName("Islands"); isl = ew.activeEffect()
                print("STEP::7/14::Removing Small Islands...")
            ...
            def visualise(seg_node, stl_path):
                ...
                print("STEP::10/14::Loading Segmentation...")
                segNode = slicer.util.loadSegmentation(seg_output_path)
                segNode.CreateClosedSurfaceRepresentation()
                slicer.app.processEvents()  # Allow closed-surface representation to be built
                sd = segNode.GetDisplayNode()
                # FIX 1: Set both overall display-node visibility AND 3D-view-specific
                # visibility so the segmentation is always visible after loading.
                # SetVisibility3D alone is not sufficient when the overall display
                # node flag is off (which is the default after loadSegmentation).
                sd.SetVisibility(True)       # overall display node switch
                sd.SetVisibility3D(True)     # 3D-view specific
                sd.SetOpacity(1.0)           # full overall opacity
                sd.SetOpacity3D(1.0)         # full opacity in 3D view
                # Explicitly enable every individual segment so none are hidden
                _seg_obj = segNode.GetSegmentation()
                for _i in range(_seg_obj.GetNumberOfSegments()):
                    _sid = _seg_obj.GetNthSegmentID(_i)
                    sd.SetSegmentVisibility(_sid, True)
                    sd.SetSegmentVisibility3D(_sid, True)
                slicer.app.processEvents()
                print("STEP::11/14::Loading STL Model...")
                ...
                print("STEP::12/14::Scaling & Translating STL...")
                ...
                print("STEP::13/14::Hardening Transform...")
                slicer.vtkSlicerTransformLogic().hardenTransform(modelNode)
                print("STEP::14/14::Launching 3D Visualization...")
                slicer.app.layoutManager().setLayout(slicer.vtkMRMLLayoutNode.SlicerLayoutOneUp3DView)
                slicer.app.processEvents()
                # FIX 1 (continued): Re-assert segmentation visibility after layout change
                # because changing the layout can reset display node flags in some
                # Slicer versions. Both flags must be set again here.
                sd.SetVisibility(True)
                sd.SetVisibility3D(True)
                slicer.app.processEvents()
        ...
        try:
                    ...
                    try:
                        for line in proc.stdout:
                            ...
                            if line.startswith('STAGE::ERROR::'):
                                ...
                                # Change 2: emoji removed
                                self.root.after(0, lambda m=line: self.tab3_log(f"WARN: {m}"))
                                continue
                            ...
                            if line.startswith('STAGE::'):
                                ...
                                continue
                            # ── Filter noisy Slicer-internal lines ──────────────
                            # These are Qt/VTK module warnings that offer no
                            # actionable information to the user (e.g. the
                            # "no main window available" message printed by
                            # the Markups module during headless runs).
                            # Add any additional noisy prefixes to this tuple
                            # to keep the status box clean.
                            _SLICER_NOISE = (
                                'qSlicerMarkupModulePrivate',
                                'qSlicerMarkupsModule',
                                'no main window is available',
                                'QStandardPaths',
                            )
                            if any(noise.lower() in line.lower()
                                   for noise in _SLICER_NOISE):
                                continue
                            self.root.after(0, lambda m=line: self.tab3_log(f"[Slicer] {m}"))
                    except Exception as _read_exc:
                        self.root.after(0, lambda e=_read_exc: self.tab3_log(
                            # Change 2: emoji removed
                            f"WARN: Lost connection to Slicer output stream: {e}"))
            ...
            def _on_viz_done(ok, err):
                # FIX 2: Always drive the progress bar to 100 % once the viz
                # phase ends, regardless of whether it succeeded or not.
                # Previously only the success branch called _set_progress(100),
                # leaving the bar frozen at its last intermediate value on
                # any error or unexpected Slicer exit.
                _set_progress(100)
                if ok and not err:
                    ...
                    # Change 2: emoji removed
                    self.tab3_log(f"ERROR: 3D Visualization failed: {err}")
                    ...
                    # Change 2: emoji removed
                    self.tab3_log("WARN: Slicer output stream closed before the pipeline reported completion. "
                                  "3D Visualization may not have finished successfully.")

            def _on_gen_done(ok, err):
                if not ok or err:
                    # FIX 2: Set progress to 100% even when generation fails so the
                    # bar never stays frozen at an intermediate value after the
                    # headless Slicer process exits.
                    _set_progress(100)
                    _set_status('3D segmentation generation failed.')
                    # Change 2: emoji removed
                    self.tab3_log(f"ERROR: Segmentation generation failed: {err or 'unknown error'} - Slicer will not be opened.")
                    messagebox.showerror('3D Visualization Error',
            ...
            def _viz_err(e=exc):
                ...
                # Change 2: emoji removed
                self.tab3_log(f"ERROR: Failed to launch 3D Slicer: {e}")
    ...
    def _batch3d_launch_viz_independent(self):
        ...
        def _launch():
            ...
            if mode == 'seg_nrrd':
                ...
                # Change 4: include indicator / non_indicator alongside legacy defects / non_defects
                has_sub = any(
                    os.path.isdir(os.path.join(results_path, s))
                    for s in ('defects', 'non_defects', 'Defects', 'Non_Defects',
                              'indicator', 'non_indicator', 'Indicator', 'Non_Indicator')
                )
                if not has_sub:
                    err_lbl.config(
    ...
    def create_step1_controls(self):
        ...
        # Change 2: emoji removed from closed status
    ...
    def tab3_start_batch_processing(self):
        ...
        # Change 4: output folders renamed to indicator / non_indicator
        defects_path     = os.path.join(output_base, "indicator")
        non_defects_path = os.path.join(output_base, "non_indicator")
        os.makedirs(defects_path, exist_ok=True)
        os.makedirs(non_defects_path, exist_ok=True)
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                ...
                for done_idx, fut in enumerate(as_completed(futures), start=1):
                    ...
                    try:
                        ...
                        if num_seg >= 1:
                            defect_count += 1
                            # Change 2: emoji removed; Change 4: folder name updated
                            msg = f"{fname} -> indicator ({num_seg} segments)"
                        else:
                            non_defect += 1
                            # Change 2: emoji removed; Change 4: folder name updated
                            msg = f"{fname} -> non_indicator"
                    except Exception as exc:
                        # Change 2: emoji removed
                        msg = f"{fname} -> ERROR: {exc}"
                    ...
                    # Throttle UI updates — only update progress bar/status label
                    # Change 3: individual per-image log lines completely suppressed
                    if done_idx % UPDATE_EVERY == 0 or done_idx == total:
                        progress = (done_idx / total) * 100
                        _di  = done_idx
                        def _update(p=progress, di=_di):
    ...
    def _rpt_generate(self):
        ...
        # Update metric tiles
        total = len(results)
        # Change 2: emojis removed from warning and status text
        warn_txt = f'  WARN: {len(warnings)} warning(s): {"; ".join(warnings[:3])}{"..." if len(warnings) > 3 else ""}' if warnings else ''
    ...
    def _rpt_evaluate(self, pred_folder, gt_folder):
        ...
        """
        # Change 4: internal category constants now use new folder names;
        # also accept the legacy 'defects'/'non_defects' names for backwards compatibility.
        DEFECT     = 'indicator'
        NON_DEFECT = 'non_indicator'

        def _load_folder(root_path):
            """Return {filename_lower: category_string} from indicator/ non_indicator/ subfolders.
            Accepts both old names (defects/non_defects) and new names for backwards compatibility."""
            mapping = {}
            for sub in os.listdir(root_path):
                ...
                # Normalise: accept old ('defects','defect') and new ('indicator') names
                if sub_lower in ('defects', 'defect', 'indicator'):
                    category = DEFECT
                elif sub_lower in ('non_defects', 'non_defect', 'non_indicator'):
                    category = NON_DEFECT
                else:
...
class SplashScreen:
    ...
    def __init__(self, root):
        ...
        # Org badge top-left
        self.canvas.create_text(28, 22, text='RCI-DAIV \u00b7  RCI-DAIV',
